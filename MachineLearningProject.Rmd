...
The Weight Lifting Exercise Dataset is used for this project.  Caret package is used to establish different models for prediction.  Random Forest appraoch with cross validation proves to be quite accurate in predicting outcomes. 
...
The first seven variables are not relevant towards predicting "classe" so were removed from the datasets.  All columns with missing values are removed too.
...
plot(trainingSet$classe, col="green", main="Levels of the variable classe within the subTraining dataset", xlab="classe levels", ylab="Frequency")
```
...
The accuracy of this model is 0.9941, and the expected out-of sample error is estimated at 0.0059.  Based on the accuracy, this model is selected as the final model.
...
Weight Lifting Exercise Dataset is used in this project.  After data cleaning by removing NAs, Blank Columns and not-related columns the variables number was successfully reduced from 160 to 53.  
